// google-business-oauth-start — Endpoint AUTENTICADO (verify_jwt = true).
// El panel llama aquí para obtener la URL de consentimiento de Google para
// conectar la ficha de Google Business Profile de UN NEGOCIO (a diferencia
// de Calendar, aquí no hay professional_id: la ficha es del negocio, no de
// un profesional concreto). Reutiliza el mismo Client ID/Secret que Google
// Calendar (mismo proyecto de Google Cloud, scope adicional), así que no
...
Deno.serve(async (req) => {
  ...
  const [{ data: profile }, { data: membership }] = await Promise.all([
    ...
    admin.from("business_users").select("id").eq("business_id", business_id).eq("user_id", uid).maybeSingle(),
  ]);
  if (!profile?.is_super_admin && !membership) return json({ error: "No autorizado" }, 403);

  const { data: integ } = await admin.from("business_integrations").select("google_client_id").eq("business_id", business_id).maybeSingle();
  ...
  const state = await signBusinessState(business_id, stateSecret);
