...
| `0029_psicologo_client_records.sql` | `customers.nif`; ficha de cliente ampliada para negocios **psicólogo**: `client_ai_reports` (informes generados por Gemini, persistidos); `business_integrations.gemini_api_key` (secreto, RLS igual que Resend/WhatsApp) + RPCs `get_gemini_status`/`set_gemini_key`. `client_notes`/`client_tasks` de esta migración se sustituyeron por `client_sessions` en `0030` (ver abajo) |
| `0030_session_records.sql` | Sustituye `client_notes`+`client_tasks` (sin datos reales, solo prueba ya limpiada) por `client_sessions`: una fila por sesión con 4 campos estructurados (`objetivo`, `notas`, `seguimiento`, `tareas_pautas`), opcionalmente ligada a una reserva (`booking_id`). `client_ai_reports.notes_count`/`tasks_count` → `sessions_count` |
...
- Migración `0029_psicologo_client_records.sql`: `customers.nif` (opcional, para el recibo), `client_ai_reports` (informes generados por IA, persistidos), y `business_integrations.gemini_api_key` + RPCs `get_gemini_status`/`set_gemini_key` (mismo patrón que Resend/WhatsApp/Google, con su propio par de RPCs para no tocar la firma de `get_business_integration`/`set_business_integration`).
- **Rediseñado tras la primera versión** (migración `0030_session_records.sql`, mismo día): `client_notes`+`client_tasks` (notas y tareas sueltas, sin relación entre sí) se sustituyeron por `client_sessions` — un modelo de "sesión" con 4 campos estructurados por registro (**Objetivo**, **Notas**, **Seguimiento** — qué revisar en próximas sesiones —, **Tareas/Pautas**), opcionalmente ligado a una reserva (`booking_id`).
- Ficha de cliente (`Panel → Clientes`) con pestañas nuevas para psicólogo: **Historial** (lista de sesiones, la más reciente primero, cada una con sus 4 campos), **Editar** (antes no existía — los datos del cliente solo se podían fijar al crearlo), **Informe** (resumen generado por Gemini a partir de todas las sesiones del Historial, vía la Edge Function `generate-client-ai-report`) y **Recibo** (lista de reservas completadas + precio + total, con botón para generar un PDF no fiscal, 100% en el navegador con `jspdf`).
- Nueva sección **Seguimiento** en el sidebar psicólogo: lista de la cita de hoy en curso ahora mismo y la siguiente (mismo cálculo por ventana de tiempo que "Plano de sala" del restaurante, sin cronómetro). Botón "Empezar cita" (o "Apuntar notas" en la siguiente) abre un formulario con los mismos 4 campos (Objetivo/Notas/Seguimiento/Tareas-Pautas); al guardar, crea una sesión en el Historial del cliente ligada a esa reserva.
