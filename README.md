...
- **Pendiente de confirmación explícita del usuario**: migrar el negocio real "Ana Sánchez Psicóloga" (hoy `type = 'citas'`) a `type = 'psicologo'` para que use esta ficha ampliada — el panel no tiene UI para cambiar el tipo tras crear el negocio, requiere un `UPDATE` manual de super-admin.
- **No verificable en vivo sin credencial externa**: todo excepto el informe de IA (Editar, Notas, Tareas, Seguimiento, recibo PDF) se puede probar de punta a punta sin nada externo. El informe de IA necesita que el usuario pegue una clave real de Gemini en un negocio de prueba.
...
| **Informe de IA con Gemini en la ficha de cliente** (Fase 9, migración `0029`, código desplegado sin probar en vivo) | Necesita que un negocio de prueba pegue una clave real de Gemini en `Configuración → Informes con IA` | Conseguir clave gratis en Google AI Studio, pegarla, generar un informe de prueba con notas/tareas ya cargadas |
| **Migrar "Ana Sánchez Psicóloga" a `type='psicologo'`** para que use la ficha de cliente ampliada (Fase 9) | Requiere confirmación explícita del usuario antes de tocar producción — el panel no tiene UI para cambiar el tipo | `UPDATE businesses SET type='psicologo' WHERE id='<id de Ana Sánchez>';` vía SQL, solo con confirmación |
